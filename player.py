...
class Player():
    ...
    def think(self, mode, box_lists, agent_position, velocity):
        ...
        # input()
        params = np.array([[velocity/11] , [box_lists[0].gap_mid/420],\
            [agent_position[1]/720], [agent_position[0]/1280 - box_lists[0].x/1280]])
        output = self.nn.forward(params)
        direction = 1 if (output >= 0.5) else -1
        return direction
